...
void getAngVelFourWheels(double vx, double vy, double wz, int *u)
{
    ...
    for (i = 0; i < 4; i++)
    {
        u[i] = (int)(((H[i][0] * twist[0] + H[i][1] * twist[1] + H[i][2] * twist[2]) / RADIUS) * (KU_avg/CONV_VAL) + .5);
    }
}
...
{
    double v1 = u1 * CONV_VAL/KU_avg;
    double v2 = u2 * CONV_VAL/KU_avg;
    double v3 = u3 * CONV_VAL/KU_avg;
    double v4 = u4 * CONV_VAL/KU_avg;
